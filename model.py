...
from shufflenet import shufflenet_my 

def get_model(model_type=None):
    if model_type == 'mlp':
        return SimpleMLP().cuda()
        ...
        net = resnet18(num_classes=11, weights=None)
        fc_features = net.fc.in_features
        net.fc = nn.Linear(fc_features, 11)
        return net.cuda()
    elif model_type == 'wideres':
        net = WideResNet(16, num_classes=10, widen_factor=2, dropRate=0.3)
        return net.cuda()
    else:
